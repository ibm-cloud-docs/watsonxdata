...
The {{site.data.keyword.lakehouse_short}} Spark instance provides REST APIs to submit and manage Spark applications. The following operations are supported:
...
Before you can submit a Spark application, you need to get authentication credentials and set the correct permissions on the {{site.data.keyword.lakehouse_short}} instance.
...
1. Generate an IAM token if you haven’t already done so.
...
1. Submit the Spark application:
    ```sh
   curl --request POST --url https://<region>.lakehouse.cloud.ibm.com/lakehouse/api/v3/spark_engines/<spark_engine_id>/applications --header "Authorization: Bearer $token" -H "content-type: application/json"  -d @submit-spark-quick-start-app.json
    ```
    {: codeblock}
...
1. Generate an IAM token if you haven’t already done so. See [Retrieving IAM access tokens](https://cloud.ibm.com/docs/account?topic=account-iamtoken_from_apikey).
1. Export the token into a variable:
    ```sh
    export token=<token generated>
    ```
...
1. Prepare the payload JSON file. For example, `submit-spark-app.json`:
    ...
    - `<cos-reference-name>` in the `"conf"` section of the sample payload is any name given to your {{site.data.keyword.cos_full_notm}} instance, which you are referencing in the URL in the `"application"` parameter.
...
1. Submit the Spark application:
    ```sh
   curl --request POST --url https://<region>.lakehouse.cloud.ibm.com/lakehouse/api/v3/spark_engines/<spark_engine_id>/applications --header "Authorization: Bearer $token" -H "content-type: application/json"  -d @submit-spark-app.json
    ```
    {: codeblock}
...
1. If forward logging was enabled for your instance, you can view the application output in the platform logs that are forwarded to {{site.data.keyword.la_full_notm}}.
...
| spark-submit command parameter | Payload to the {{site.data.keyword.lakehouse_short}} Spark submission REST API|
|----------------------------------|---------------------------------------|
...
```sh
curl -X GET https://<region>.lakehouse.cloud.ibm.com/lakehouse/api/v2/spark_engines/<spark_engine_id>/applications/<application_id>/state --header "Authorization: Bearer $token"
```
{: codeblock}
...
```sh
curl -X GET https://<region>.lakehouse.cloud.ibm.com/lakehouse/api/v2/spark_engines/<spark_engine_id>/applications/<application_id> --header "Authorization: Bearer $token"
```
{: codeblock}
...
```sh
curl -X DELETE https://<region>.lakehouse.cloud.ibm.com/lakehouse/api/v2/spark_engines/<spark_engine_id>/applications/<application_id> --header "Authorization: Bearer $token"
```
{: codeblock}
