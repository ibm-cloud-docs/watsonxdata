...
4. From the left navigation pane, select **Access control**. Click **New access token** tab and click **New access token**.
...
5. From the watsonx.ai project, click **Assets** tab. Click **New asset**.
...
9. Click **Test connection** to make sure the connection works. If your test is successful, `The test was successful` message is displayed.
...
3. Add the Jupyter Notebook (`watsonx.data RAG Notebook`). To add:
   ...
   `https://raw.github.ibm.com/PrivateCloud/watsonx-data-public/main/Tutorials/watsonxdata-RAG-tutorial.ipynb?token=GHSAT0AAAAAAACDLLADCIN4ZDM4SVPXF4ZKZTMFBMQ`
   .
...
: Use a sentence transformer model (in this use case, `sentence-transformers/all-MiniLM-L6-v2`) to vectorize the data chunks. Define a collection and store the vectorized data into the collection.
