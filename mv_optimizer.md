...
1. Set the property for `enable-query-optimizer-materialized-view` to `true` to enable Materialized View (MV) feature at global level using patch API.

   See the following patch API curl example. For detailed information on customizing API properties, see [API customization](https://cloud.ibm.com/docs/watsonxdata?topic=watsonxdata-api_custom_ov).
...
2. Identify and select queries that are complex, frequently used, and involve joins or aggregations.
