...
As a workaround, you can provide select privilege for the schema that the user created.
...
As a workaround, you can provide select privilege for the table that the user created.

## Issue: Longer run time for running SQL queries from SQL editor
{: #known_issues3}

It takes more than 30 seconds to run an SQL query by using the SQL editor in the stand-alone version. For example, the SQL editor spends more than 30 seconds to display the tables from the schema when you create thousand tables under a schema.
...
## Issue: Presto does not recognise the path as a directory and an error might occur
{: #known_issues12}

When you create a new table with a Presto Hive connector that uses an S3 folder from an external location, presto does not recognise the path as a directory and an error might occur.
...
As a workaround, you must make sure that the location of the bucket is specified while creating a schema.
...
As a workaround, you must make sure to create unique names while creating schemas and buckets.
...
As a workaround, if you see any of the define class error or name or service not known error, you must wait for a few minutes for the server to completely restart. Then, retry the query for successful execution.
...
As a workaround, it is recommended to use the third and fourth options. The fourth option is best recommended for better structuring. The first two options are not to be used as it results in a failure.

## Issue: Adding database memory gives error
{: #known_issues20}

Creating a database of type Memory, gives the error, `pq: invalid input syntax for type integer: ""`.

## Issue: Presto does not support `AS OF` with iceberg tables
{: #known_issues21}

- Presto does not support `AS OF <time stamp>` in a SELECT query.
- As a workaround, invoke CALL iceberg_data_rollback_to_snapshot’ to move to the desired timestamp.
...
## Issue: Only the creator has DROP privilege access on the table in Apache hive (API)
{: #known_issues22}
...
As a workaround, use the presto-cli to do the CTAS operation and set the following session parameter before the CTAS query:
...
For details about connecting to a Presto server through a CLI, see [Connecting to a Presto Server](watsonxdata?topic=watsonxdata-conn-presto-thr-cli).
{: note}
...
As a workaround, you must first list the folders inside the bucket using `aws s3 ls` command. If '0' empty file is listed, you must copy all the files to another folder using `aws s3 cp` command.
